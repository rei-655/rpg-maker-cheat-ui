...
\`node cheat-ui/tools/install.mjs "/path/to/Game"\`.

**cheat-ui-uninstall.bat** removes it the same way.

These files are safe to unpack straight into a game folder: everything is
prefixed with \`cheat-ui\`, so nothing of the game is overwritten. The installer
...
それ以外の環境では \`node cheat-ui/tools/install.mjs "/path/to/Game"\` を使います。

削除は **cheat-ui-uninstall.bat** です。

ゲームのフォルダにそのまま展開しても構いません。名前はすべて \`cheat-ui\` で
始まるため、ゲームのファイルを上書きしません。\`js/main.js\` には触れず、
